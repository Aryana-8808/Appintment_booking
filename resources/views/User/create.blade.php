...
<body>
    <div class="container form-container">
        <div class="card">
            ...
            <div class="card-body">
                @if (session('error'))
                    <form action="{{ route('bookings.addToWaitlist') }}" method="post" id="appointmentForm">
                    @else
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('slot_id') is-invalid @enderror" id="slot_id" name="slot_id"
                        required>
                        @foreach ($availableSlots as $slot)
                            ...
                            @endphp
                            <option value="{{ $slot->id }}" data-start="{{ $slot->start_time }}"
                                data-end="{{ $slot->end_time }}" class="{{ $class }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('phone') is-invalid @enderror" id="phone"
                        name="phone" pattern="[0-9]{10,}" placeholder="Phone number" value="{{ old('phone') }}"
                        required>
                    @error('phone')
                        <p class="invalid-feedback">{{ $message }}</p>
                    @enderror
