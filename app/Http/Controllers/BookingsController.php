...
class BookingsController extends Controller
{
  ...
  public function isAvailable($appointment_date, $duration)
  {
    ...
    return !Booking::where('appointment_date', '<', $end_date)->where('end_time', '>', $appointment_date)->exists();
  }
  ...
  public function create()
  {
    ...
    $availableSlots = AppointmentSlot::get();
  ...
  public function store(Request $request)
  {
    try {
      ...
      $overlappingBooking = Booking::where('appointment_date', '<', $appointmentEnd)->where('end_time', '>', $appointmentStart)->exists();
      if ($overlappingBooking) {
        ...
        if (!$nearestSlot) {
          return redirect()->route('bookings.create')->with('error', 'Booking cannot be made. There is already an overlapping booking and no available slots on the same day. You can join waitlist.');
        }

        return redirect()
          ->route('bookings.create')
          ->with('error', 'Booking cannot be made. There is already an overlapping booking. Nearest available slot: ' . $nearestSlot->start_time . ' - ' . $nearestSlot->end_time . '. You can join waitlist.');
      }
      ...
      $booking->save();
      // toggle appointment slot availability
      $appointment = AppointmentSlot::findOrFail($request->slot_id);
  ...
  public function findNearestAvailableSlot($appointmentDate, $requestedDuration)
  {
    ...
    $slots = AppointmentSlot::where('available', true)->where('start_time', '>=', $startTime)->where('end_time', '<=', $endTime)->orderBy('start_time')->get();
  ...
  private function checkAvailability($appointmentDate, $slotStartTime, $slotEndTime, $requestedDuration)
  {
    ...
    return !Booking::where('appointment_date', '<', $appointmentEnd)->where('end_time', '>', $appointmentStart)->exists();
  }
  ...
  public function update($id, Request $request)
  {
    ...
    if ($overlappingBooking) {
      ...
      return redirect()
        ...
        ->with('error', 'Booking cannot be made. There is already an overlapping booking. Nearest available slot: ' . $nearestSlot->start_time . ' - ' . $nearestSlot->end_time);
    }
  ...
  public function addToWaitlist(Request $request)
  {
    ...
    // Create new booking instance with waitlist
    $booking = new Booking();
    $booking->user_id = $request->user_id;
    $booking->slot_id = $request->slot_id;
    $booking->name = $request->name;
    ...
    $booking->description = $request->description;
    $booking->save();
    return redirect()->route('account.dashboard')->with('success', 'You have been added to the waitlist.');
  }
